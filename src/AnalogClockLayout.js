...
function renderNotches({ smallTick, largeTick }, width) {
    const notches = [];
    // console.log('width', width)

    for (let i = 0; i < 60; i++) {
        var x =  width / 2 + width / 2 * Math.sin(i * 6 * (Math.PI / 180));
        var y =  width / 2 - width / 2 * Math.cos(i * 6 * (Math.PI / 180));
        let style = Object.assign({}, i % 5 === 0 ? largeTick : smallTick, {
            // transform: `translateX(-50%) translateY(-100%) rotate(${i * 6}deg)`,
            transform: `translateX(-50%) translateY(-50%) rotate(${i * 6}deg)`,
            left: x,
            top: y
        });
...
function renderHours({ digits }, width) {
    const notches = [];
    // console.log('width', width)

    for (var i = 30; i <=360; i=i+30) {
        var x =  width / 2 + width / 2 * 0.7 * Math.sin(i * (Math.PI / 180));
        var y =  width / 2 - width / 2  * 0.7 * Math.cos(i * (Math.PI / 180));
        let style = Object.assign({}, digits, {
            left: x,
            top: y
        });
        console.log('xxx', i/30)
        const hour = i/30;
        notches.push(<span key={i} style={style}>{hour}</span>);
    }
    return notches;
}
...
AnalogClockLayout.propTypes = {
    ...
    styles: PropTypes.shape({
        ...
        digits: PropTypes.object.isRequired
    }).isRequired,
